...
\end{align*}
Fortunately, with the measurement equations, we can say things. Recall $I=E[Y_1 - Y_0 - C|X,Z,\theta]$. \textbf{I have no idea what to do with the last 2 eqns b/c how do we compute $I$ w/ out $\theta$ ??}
\begin{align*}
...
\end{align*}
The top four equations give us two measurements for $\alpha_B$. We also use the first and third along with $\rho \sigma_\theta$ to get $\alpha_c \sigma^2_\theta$. We are still not able to pull apart $\alpha_0/\alpha_1$, which means we are having trouble getting $\sigma^2_\theta$ and $\sigma^2_{k}$.
